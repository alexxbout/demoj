...
options=("Install DemoJ on this device" "Run appservice" "Run repository" "Run sudoers" "Run virtualenv" "Run staticip" "Run demojconnect" "Run raspios" "Run raspap" "Exit")
select install_option in "${options[@]}"; do
    case $REPLY in
        ...
        4) user=$(select_user); "others/repository.sh" "$user"; break;;
        5) user=$(select_user); "others/sudoers.sh" "$user"; break;;
        6) user=$(select_user); "others/virtualenv.sh" "$user"; break;;
        7) "others/staticip.sh"; break;;
        8) "others/demojconnect.sh"; break;;
        9) "others/raspios.sh"; break;;
        10) "others/raspap.sh"; break;;
