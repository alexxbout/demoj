...
BEGIN_COLOR = Color(255, 0, 0)
ENDING_COLOR = Color(0, 0, 255)
...
def wipe(strip: PixelStrip, color: Color):
    ...
    for i in range(pixels):
        ...
        strip.show()
        time.sleep(0.100) # For something animated
...
def degrade(i: int, max: int, begin: Color, end: Color): 
    percent = float(i / max)
    red = begin.r() - (end.r() * percent)
    green = begin.r() - (end.r() * percent)
    blue = begin.r() - (end.r() * percent)
